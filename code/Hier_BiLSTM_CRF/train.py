...
def learn(model, x, y, tag2idx, val_fold, args):
    dataset_size = len(x)
    assert dataset_size >= args.num_folds, "Dataset size must be >= num_folds"
    
    samples_per_fold = (dataset_size + args.num_folds - 1) // args.num_folds  
    val_start = val_fold * samples_per_fold
    val_end = min(val_start + samples_per_fold, dataset_size)  
    val_idx = list(range(val_start, val_end))
    train_idx = [i for i in range(dataset_size) if i not in val_idx]

    train_x, train_y = [x[i] for i in train_idx], [y[i] for i in train_idx]
    val_x, val_y = [x[i] for i in val_idx], [y[i] for i in val_idx]
